...
The `template/` directory is a git submodule pointing to
`rhpds/rhdp-publishing-house-template`. Changes made in `template/` are committed
directly to the template repo through the submodule.
